...
async def on_message(message):
    ...
    # Only moderate in configured servers
    if guild_id not in ALLOWED_GUILDS:
        return
    ...
    if detected_word:
        try:
            ...
            await message.author.timeout(duration, reason="Automatic timeout: Used inappropriate language")

            # Create embed for auto-timeout
            embed = discord.Embed(
                title="🚫 Auto-Moderation: Inappropriate Language",
                description=f"{message.author.mention} has been automatically timed out",
                color=0xff4444,  # Darker red for serious violation
            ...
            embed.add_field(name="Reason", value="Used inappropriate language", inline=True)
            ...
            embed.set_footer(text=f"User ID: {message.author.id} | Message deleted automatically")

            # Send the embed to the channel where the violation occurred
            await message.channel.send(embed=embed)

            # Optional: Log to a specific mod channel if you have one
            # mod_channel = bot.get_channel(YOUR_MOD_CHANNEL_ID)
            # if mod_channel:
            #     log_embed = embed.copy()
            #     log_embed.add_field(name="Original Message", value=f"||{message.content[:100]}{'...' if len(message.content) > 100 else ''}||", inline=False)
            ...
            warning_embed = discord.Embed(
                ...
                description=f"{message.author.mention} used inappropriate language, but I couldn't timeout the user.",
                color=0xff9900
            )
            warning_embed.add_field(name="Action Required", value="Manual moderation needed", inline=False)
            await message.channel.send(embed=warning_embed)

        except Exception as e:
            print(f"Error in auto-moderation: {e}")

    # Don't forget to process commands after checking the message
    await bot.process_commands(message)
...
# Optional: Command to toggle auto-moderation (only for master/authorized users)
@bot.command(name="automod")
async def toggle_automod(ctx, status: str = None):
    """Enable or disable auto-moderation for inappropriate language"""
    if ctx.guild is None:
        return await ctx.send("❌ This command must be used in a server (not DMs).")
    ...
    # Only allow master or authorized users to toggle
    allowed_users = GUILD_USER_MAP.get(guild_id, [])
    if author_id != MASTER_ID and author_id not in allowed_users:
        error_embed = discord.Embed(
            title="❌ Access Denied",
    ...
    if guild_id not in ALLOWED_GUILDS:
        error_embed = discord.Embed(
            ...
            description="This server is not configured for moderation commands.",
            color=0xff0000
        )
        return await ctx.send(embed=error_embed)
    ...
    info_embed = discord.Embed(
        ...
        description="Auto-moderation is currently **ENABLED** for inappropriate language.",
        color=0x00ff00
    )
    info_embed.add_field(name="Current Action", value="24-hour timeout + message deletion", inline=False)
    info_embed.add_field(name="Coverage", value="Detects N-word variations automatically", inline=False)
    info_embed.add_field(name="Exemptions", value="Master user is exempt from auto-moderation", inline=False)
